...
public class QueryServiceImpl implements IQuerySerivce {
	...
	private PlayerData setPlayerInfo(PlayerEntity playerEntity) {
		...
		playerData.setInfoData(new PlayerInfoData()
				...
				.setTeamName(TeamName_2021.getTeamNameFromId(playerEntity.getTeamId()).name())
	...
	private void setPlayerFixture(PlayerData playerData) {
		...
		IntStream.range(currentEvent, currentEvent + 6).forEach(event -> {
			...
			eventFixtureEntityList.forEach(eventFixtureEntity -> {
				...
				fixtureDataList.add(new PlayerFixtureData()
						.setEvent(event)
						.setPlayAgainstTeam(wasHome ?
								TeamName_2021.getTeamNameFromId(teamId).name() : TeamName_2021.getTeamNameFromId(eventFixtureEntity.getTeamA()).name())
	...
	private void setCurrentSeasonData(PlayerData playerData, PlayerEntity playerEntity) {
		...
		BeanUtil.copyProperties(playerEntity, playerCurrentData, CopyOptions.create().ignoreNullValue());
		...
		if (playerValueEntity != null) {
			BeanUtil.copyProperties(playerValueEntity, playerCurrentData, CopyOptions.create().ignoreNullValue());
		}
		playerData.setCurrentSeasonData(playerCurrentData);
	}

	private void setAllHistorySeasonData(PlayerData playerData) {
		Arrays.stream(HistorySeason.values()).forEach(o -> {
			List<PlayerHistoryData> historySeasonDataList = Lists.newArrayList();
			PlayerHistoryData playerHistoryData = this.setHistorySeasonData(o.getSeason(), playerData);
			if (playerHistoryData != null) {
				historySeasonDataList.add(playerHistoryData);
			}
			playerData.setHistorySeasonDataList(historySeasonDataList);
		});
	}

	private PlayerHistoryData setHistorySeasonData(String season, PlayerData playerData) {
		MybatisPlusConfig.season.set(season);
