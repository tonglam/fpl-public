...
public class TableQueryServiceImpl implements ITableQueryService {
    ...
    //    @Cacheable(value = "qryScoutPlayerList", key = "#elementType+'::'+#page+'::'+#limit")
    @Override
    public TableData<PlayerData> qryPageScoutPlayerList(int elementType, int page, int limit) {
        ...
        Page<PlayerEntity> playerPage = this.playerService.getBaseMapper().selectPage(
                new Page<>(page, limit, true), new QueryWrapper<PlayerEntity>().lambda()
                        .eq(PlayerEntity::getElementType, elementType));
        playerPage.getRecords().forEach(o ->
                list.add(this.queryService.qryPlayerData(o.getElement())));
        Page<PlayerData> pageResult = new Page<>(page, limit, playerPage.getTotal());
        pageResult.setRecords(list);
        return new TableData<>(pageResult);
    }
