...
public class TableQueryServiceImpl implements ITableQueryService {
    ...
    @Cacheable(value = "qryZjTournamentResultById", key = "#tournamentId", unless = "#result == null")
    @Override
    public TableData<ZjTournamentResultData> qryZjTournamentResultById(int tournamentId) {
        ...
        IntStream.range(1, groupNum + 1).forEach(groupId -> {
            ...
            // step
            int step = 0;
            if (phaseOneTotalPoints != 0) {
                step = 1;
            } else if (phaseTwoTotalPoints != 0) {
                step = 2;
            }
            zjTournamentResultData
                    .setStep(step)
                    .setTournamentTotalPoints(zjTournamentResultData.getPhaseOneTotalPoints() + zjTournamentResultData.getPhaseTwoTotalPoints() + zjTournamentResultData.getPkTotalPoints())
                    .setTournamentTotalGroupPoints(zjTournamentResultData.getPhaseOneTotalGroupPoints() + zjTournamentResultData.getPhaseTwoTotalGroupPoints() + zjTournamentResultData.getPkTotalGroupPoints());
            list.add(zjTournamentResultData);
        });
        // group tourmanet rank
        Map<String, Integer> tournamentRankMap = this.querySerivce.qryZjTournamentRankMap(list);
        list.forEach(o -> o.setTournamentRank(tournamentRankMap.getOrDefault(String.valueOf(o.getGroupId()), 0)));
        return new TableData<>(list
                .stream()
    ...
    private Map<String, Integer> setPhaseOneTotalGroupPoints(Map<String, Integer> phaseOneRankMap) {
        ...
        phaseOneRankMap.forEach((groupId, rank) -> map.put(groupId, this.setRankPoints(rank)));
        return map;
    }

    private Map<String, Integer> setPhaseTwoTotalGroupPoints(Map<String, Integer> phaseTwoRankMap) {
        Map<String, Integer> map = Maps.newHashMap();
        phaseTwoRankMap.forEach((groupId, rank) -> map.put(groupId, this.setRankPoints(rank)));
        return map;
    }

    private Map<String, Integer> setPkTotalGroupPoints(Map<String, Integer> pkRankMap) {
        Map<String, Integer> map = Maps.newHashMap();
        pkRankMap.forEach((groupId, rank) -> map.put(groupId, this.setRankPoints(rank)));
        return map;
    }

    private int setRankPoints(int rank) {
        switch (rank) {
            case 1:
                return 5;
            case 2:
