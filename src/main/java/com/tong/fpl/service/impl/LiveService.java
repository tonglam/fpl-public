...
public class LiveService implements ILiveService {
    ...
    private Table<Integer, Integer, Integer> getLiveBonusTable() {
        ...
        Map<String, Map<String, Integer>> liveBounsMap = this.queryService.getLiveBonusCacheMap();
        if (CollectionUtils.isEmpty(liveBounsMap)) {
            return table;
        }
        liveBounsMap.keySet().forEach(teamId ->
                liveBounsMap.get(teamId).forEach((element, bonus) ->
