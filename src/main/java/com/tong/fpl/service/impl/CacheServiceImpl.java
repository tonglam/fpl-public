...
public class CacheServiceImpl implements ICacheSerive {
    ...
    public void insertTeam() {
        ...
        result.ifPresent(staticRes -> {
            ...
            RedisUtils.removeCacheByKeyPattern(key);
    ...
    private void setEventFixtureCacheByTeam(String season, List<EventFixtureEntity> fixtureList) {
        RedisUtils.removeCacheByKeyPattern(StringUtils.joinWith("::", EventFixtureEntity.class.getSimpleName(), season));
        Map<String, Map<Object, Double>> cacheMap = Maps.newHashMap();
    ...
    public void insertPlayerStat() {
        ...
        result.ifPresent(staticRes -> {
            // insert table
            this.playerStatService.remove(new QueryWrapper<PlayerStatEntity>().eq("1", 1));
            ...
            log.info("insert player_stat size is " + playerStatList.size() + "!");
            // set cache
            Map<String, Map<Object, Double>> cacheMap = Maps.newHashMap();
            String key = StringUtils.joinWith("::", PlayerStatEntity.class.getSimpleName(), CommonUtils.getCurrentSeason());
            Map<Object, Double> valueMap = Maps.newConcurrentMap();
            RedisUtils.removeCacheByKeyPattern(key);
            playerStatList.forEach(o -> valueMap.put(JsonUtils.obj2json(o), (double) o.getElement()));
            cacheMap.put(key, valueMap);
    ...
    private void updatePriceOfPlayer(List<PlayerValueEntity> playerValueList) {
        ...
        playerValueList.forEach(o -> {
            ...
            // set cache
            String key = StringUtils.joinWith("::", PlayerEntity.class.getSimpleName(), CommonUtils.getCurrentSeason());
            RedisUtils.updateZsetValue(key, playerEntity, o.getElement());
        });
