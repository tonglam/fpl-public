...
public class RedisCacheServiceImpl implements IRedisCacheSerive {
	...
	private Set<Object> setEventFixtureValueByTeam(Collection<EventFixtureEntity> fixtureList, int teamId) {
		...
		fixtureList.stream()
				...
				.forEach(o -> valueSet.add(new TeamFixtureData()
						.setTeamId(teamId)
						.setEvent(o.getEvent())
	...
	@Override
	public List<TeamFixtureData> getEventFixtureByTeamId(String season, int teamId) {
		List<TeamFixtureData> list = Lists.newArrayList();
		String key = StringUtils.joinWith("::", EventFixtureEntity.class.getSimpleName(), season, "teamId", teamId);
		Set<Object> set = this.redisTemplate.opsForSet().members(key);
		if (CollectionUtils.isEmpty(set)) {
			return list;
		}
		set.forEach(o -> list.add((TeamFixtureData) o));
		return list.stream().sorted(Comparator.comparing(TeamFixtureData::getEvent)).collect(Collectors.toList());
	}
