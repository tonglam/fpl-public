...
public class RedisCacheServiceImpl implements IRedisCacheSerive {
	...
	public void insertLiveFixtureCache() {
		...
		Table<Integer, MatchPlayStatus, List<LiveFixtureData>> table = this.getEventFixtureByEvent(CommonUtils.getCurrentSeason(), event)
				.stream()
				.collect(new LiveFixtureCollector(this.getTeamNameMap()));
		Map<String, Object> valueMap = Maps.newHashMap();
		...
		RedisUtils.removeCacheByKey(key);
		cacheMap.put(key, valueMap);
		RedisUtils.pipelineHashCache(cacheMap, 1, TimeUnit.HOURS);
	}
	...
	public void insertEventLive(int event) {
		...
		cacheMap.put(key, valueMap);
		RedisUtils.pipelineHashCache(cacheMap, 1, TimeUnit.DAYS);
	}
	...
	public Map<Integer, LiveFixtureData> getEventLiveFixtureMap() {
		String key = StringUtils.joinWith("::", LiveFixtureData.class.getSimpleName());
		return (Map<Integer, LiveFixtureData>) this.redisTemplate.opsForValue().get(key);
	}
