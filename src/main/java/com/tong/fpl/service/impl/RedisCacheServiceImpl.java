...
public class RedisCacheServiceImpl implements IRedisCacheSerive {
	...
	public Optional<UserPicksRes> getUserPicks(int event, int entry) {
		Optional<UserPicksRes> result = this.interfaceService.getUserPicks(event, entry);
		result.ifPresent(userPicksRes -> {
			// set cache
			String key = StringUtils.joinWith("::", UserPicksRes.class.getSimpleName(), event, entry);
			RedisUtils.removeCacheByKey(key);
			Map<String, Object> cacheMap = Maps.newHashMap();
			cacheMap.put(key, userPicksRes);
			RedisUtils.pipelineValueCache(cacheMap, 7, TimeUnit.DAYS);
		});
		return result;
	}
