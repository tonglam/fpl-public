...
public class HttpApiImpl implements IHttpApi {
    ...
    private int getElementByQueryParam(PlayerQueryParam queryParam) throws Exception {
        ...
        if (queryParam.getCode() > 0) {
            return this.querySerivce.qryPlayerElementByCode(season, queryParam.getCode());
        }
        if (StringUtils.isNoneBlank(queryParam.getWebName())) {
            return this.querySerivce.qryPlayerElementByWebName(season, queryParam.getWebName());
        }
        return 0;
    }
    ...
    public String qryDeadlineByEvent(int event) {
        return this.redisCacheSerive.getDeadlineByEvent(CommonUtils.getCurrentSeason(), event);
    }
