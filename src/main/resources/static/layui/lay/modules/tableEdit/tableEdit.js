layui.define(["laydate", "laytpl", "table", "layer"], function (exports) {
    ...
    var inFunc = function () {
        singleInstance.leaveStat = false;
    }, outFunc = function () {
        singleInstance.leaveStat = true;
    };
    document.onclick = function () {
        if (singleInstance.leaveStat) singleInstance.deleteAll();
    };
    ...
    Class.prototype.date = function (options) {
        ...
        laydate.render({
            elem: input[0], type: othis.dateType, show: true, done: function (value, date) {
                othis.deleteAll();
    ...
    Class.prototype.signedInput = function (options) {
        ...
        if (39 - that.offsetHeight > 3) {
            ...
            leftBtn.css('top', '16px');
            rightBtn.css('top', '16px');
        }
        if (that.offsetHeight - 39 > 3) {
            input.css('height', '50px');
            leftBtn.css('top', '25px');
            rightBtn.css('top', '25px');
        }
        input.val(othis.oldValue);
        $(that).append(leftBtn);
        leftBtn.find('i').html('');
    ...
    Class.prototype.register = function (options) {
        ...
        tableEdit.append(laytpl(othis.enabled ? selectMoreTpl : selectTpl).render({
            data: othis.data,
            style: style,
            callbackFn: getClassFn
        }));
        ...
        var searchFunc = function (val) { //关键字搜索
            ...
            liArr.unbind('click'), liArr.bind('click', function (e) {
                ...
                if (othis.enabled) {//多选
                    ...
                    othis.deleteAll();
                    if (othis.callback) othis.callback.call(othis.element, {
                        name: $(this).data("name"),
            ...
            $("div.layui-tableEdit-div button").bind('click', function () {
                var eventType = $(this).attr("event-type"), btn = this, dataList = [];
    ...
    var AopEvent = function (cols) {
        this.config = {colsConfig: {}};
        this.parseCols(cols)
    };//aop构造器
    /**
    ...
    AopEvent.prototype.on = function (event, callback) {
        var othis = this;
        othis.config.event = event, othis.config.callback = callback;
    ...
    var active = {
        aopObj: function (cols) {
            return new AopEvent(cols);
        },
